...
function DomManager() {
  ...
  this.updateList = (items, idActive) => {
    this.clearList();
    this.showList(items, idActive);
  }
}
...
window.addEventListener("load", (event) => {
  ...
  for (const navFilter of navFilters) {
    navFilter.addEventListener('click', (event) => {
      domManager.updateList(filmLib.getLibraryFiltered(navFilter.id), navFilter.id);
    })
  }

  domManager.updateList(filmLib.getLibraryFiltered("all"), "all");
});
